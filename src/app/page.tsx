...
const handleLogin = (e: React.FormEvent) => {
  ...
  const storedUser = localStorage.getItem(email);
    if (!storedUser) {
      setErrorMessage("Email ID not detected. Kindly Sign Up for an account.");
      return;
    }
    ...
    if (parsedUser.password !== password) {
      ...
      setSuccessMessage("Email Account Created, and Successfully Signed In!");
      setErrorMessage("");
      ...
      <div className="w-full h-screen flex justify-center items-center bg-black bg-opacity-80 px-4 sm:px-0">
        ...
        <h1 className="text-center text-2xl sm:text-3xl font-bold bg-orange-500 text-black rounded-t-lg m-0 py-3 sm:py-4">Sign In</h1>
          <form onSubmit={handleLogin} className="p-6 sm:p-8">
            <input type=" text" name="" placeholder="Email id" value={email} onFocus={clearErrorMessage} onChange={(e) => setEmail(e.target.value)} className="py-2 px-3 w-full text-black text-lg font-light outline-none border border-black "/>
            <input type="text" name="" placeholder="Password" value={password} onFocus={clearErrorMessage} onChange={(e) => setPassword(e.target.value)} className="py-2 px-3 w-full text-black text-lg font-light outline-none border border-black mt-4 sm:mt-5" />
